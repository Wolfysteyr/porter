                ...
                <div className="side-menu-content">
                            ...
                            <>
                                ...
                                <div style={{ marginTop: 8 }}>
                                    <Select
                                        ...
                                        onChange={(opt) => { setSelectedTable && setSelectedTable(opt ? opt.value : ''); }}
                    ...
                    <div className={`rules-section` + (selectedTable ? ' open' : '')}>
                        ...
                        <div className={`rule-item` + (menus && menus["column-menu"] ? ' open' : '')} onClick={() => toggleMenus && toggleMenus("column-menu")} >
                            ...
                            <strong>{menus && menus["column-menu"] ? "<" : ">"}</strong>
                        </div>
                        <div className={`rule-submenu` + (menus && menus["column-menu"] ? ' open' : '')}>
                            {Array.isArray(tableCols) && tableCols.map((col) => {
                                const fk = Array.isArray(Object.values(foreignKeys || {})) ? Object.values(foreignKeys).find(fk => fk.column_name === col) : undefined;
                                return (
                                    <div key={col} className="column-item">
                                        ...
                                        {fk && isToggled && isToggled(fk.constraint_name) && (
                                            <div className="nested" style={{ marginLeft: "16px" }}>
                                                ...
                                                {isToggled && isToggled(`${fk.constraint_name}-table`) && (
                                                            ...
                                                            fkcol === fk.referenced_column ? null : (
                                                                <div key={i} className="fk-details">
                                                                    <label>
                                                                        <input
                                                                            type="checkbox"
                                                                            checked={Array.isArray(selectedRFKs && selectedRFKs[fk.column_name]) && selectedRFKs[fk.column_name].includes(fkcol)}
                                                                            onChange={() => handleFKSelection && handleFKSelection(fk.column_name, fk.referenced_table, fkcol)}
                                                                        />
                                                                        {fkcol}
                                                                    </label>
                                                                </div>
                        ...
                        <div className={`rule-item` + (menus && menus["where-menu"] ? ' open' : '')} onClick={() => toggleMenus && toggleMenus("where-menu")}>
                            <label>Where Statements ({Array.isArray(selectedWhere) && selectedWhere.length > 0 ? selectedWhere.length : "none"})</label> <strong>{menus && menus["where-menu"] ? "<" : ">"}</strong>
                        </div>
                        <div className={`rule-submenu` + (menus && menus["where-menu"] ? ' open' : '')}>
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <>
                                            {selected.map((row, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                                                        <select value={currentCol} onChange={(e) => handleSelectedWhere && handleSelectedWhere(e, idx)} >
                                                        ...
                                                        {currentCol && (
                                                            <select
                                                                value={row.operator ?? '='}
                                                                onChange={(e) => handleWhereOperatorChange && handleWhereOperatorChange(idx, e.target.value)}
                                                            >
                                                        ...
                                                        {currentCol && !(row.operator === 'IS NULL' || row.operator === 'IS NOT NULL') && (
                                                            <input
                                                                ...
                                                                placeholder="value"
                                                                value={row.value ?? ''}
                                                                onChange={(e) => handleWhereValueChange && handleWhereValueChange(idx, e.target.value)}
                                                            />
                                                        )}
                                                    </div>
                                                );
                                            ...
                                            <select key="extra" value="" className="blankSelect" onChange={(e) => handleSelectedWhere && handleSelectedWhere(e, selected.length)}>
                                                <option value="" >-- Choose Column --</option>
                        ...
                        <div className={`rule-item` + (menus && menus["export-menu"] ? ' open' : '')} onClick={() => {
                            toggleMenus && toggleMenus("export-menu"); (async () => {
                                const grouped = await populateFindOptions();
                                setFindOptions && setFindOptions(grouped || {});
                            })();
                        }}>
                            <label>Export</label> <strong>{menus && menus["export-menu"] ? "<" : ">"}</strong>
                        </div>
                        <div className={`rule-submenu-export` + (menus && menus["export-menu"] ? ' open' : '')}>
                            <h3>Export Rules</h3>
                            <Tippy
                                ...
                                content={exportType ? (
                                    <div>
                                        ...
                                        <br />
                                        <button onClick={() => setShowWarning && setShowWarning(false)}>OK</button>
                                        <button onClick={() => { setShowWarning && setShowWarning(false); toggleExportType && toggleExportType(); }}>Cancel</button>
                                    </div>
                                ) : ''}>
                                <div>
                                    ...
                                    <Switch
                                        onChange={() => {
                                            ...
                                            setShowWarning && setShowWarning(!exportType);
                                        }}
                                        checked={exportType}
                                        uncheckedIcon={false}
                                        checkedIcon={false}
                            ...
                            {exportType && (
                                <div style={{ marginTop: '0.5rem' }}>
                                    ...
                                    <Select
                                        options={(databases || [])
                                            .filter(d => d.name !== selectedDatabase)
                                            .map(d => ({ value: d.name ?? d, label: d.name ?? d }))}
                                        value={targetDatabase ? { value: targetDatabase, label: targetDatabase } : null}
                                        onChange={(opt) => { const val = opt ? opt.value : ''; setTargetDatabase && setTargetDatabase(val); setTargetTable && setTargetTable(''); }}
                                    ...
                                    {targetDatabase && (
                                        <>
                                            ...
                                            <Select
                                                ...
                                                onChange={(opt) => { setTargetTable && setTargetTable(opt ? opt.value : ''); }}
                                                styles={{ menu: (provided) => ({ ...provided, zIndex: 9999, backgroundColor: '#424242', color: '#fff' }), control: (provided) => ({ ...provided, margin: "1rem", backgroundColor: '#424242', color: '#fff' }), singleValue: (provided) => ({ ...provided, color: '#fff' }) }}
                                            />
                                        </>
                                    )}
                            ...
                            <button className='add-rule-button' onClick={addLimitOffset} disabled={limitOffsetRules && limitOffsetRules.length >= 1}>Add Limit/Offset</button> <span />
                            <button className='add-rule-button' onClick={() => { setShowColumnWindow && setShowColumnWindow(true); setColumnNameChanges && setColumnNameChanges(prev => [...prev, { original: '', new: '' }]) }} disabled={showColumnWindow}>Change Column Names</button> {!showColumnWindow && exportType && <span className='attention'>!</span>}

                            {FRRules && FRRules.map((rule, index) => (
                            ...
                            {limitOffsetRules && limitOffsetRules.map((rule, index) => (
                            ...
                            {showColumnWindow && (
                                <div className="column-name-change-container">
                                    ...
                                    <button onClick={() => {
                                        setShowColumnWindow && setShowColumnWindow(false);
                                    ...
                                    <div style={{ marginTop: '0.75rem' }}>
                                        {(!columnNameChanges || columnNameChanges.length === 0) && <div style={{ color: '#999' }}>No columns added yet.</div>}

                                        {columnNameChanges && columnNameChanges.map((nameChange, index) => (
                                        ...
                                        {targetDatabase && targetTable || !exportType ? (
                                            <button onClick={() => setColumnNameChanges && setColumnNameChanges(prev => [...prev, { original: '', new: '' }])}>Add Column</button>
                                        ) : (
                            ...
                            <div className="automation-settings">
                                {isAutomated && (
                                    <>
                                        ...
                                        <select value={automationSchedule} defaultValue={"every"} onChange={(e) => setAutomationSchedule && setAutomationSchedule(e.target.value)} disabled={!isAutomated}>
                                        ...
                                        {automationSchedule === 'every' && isAutomated && (
                                            <>
                                                <input type="number" placeholder="Period" value={automationPeriod} onChange={(e) => setAutomationPeriod && setAutomationPeriod(e.target.value)} />
                                                <select defaultValue={"minutes"} value={automationUnit} onChange={(e) => setAutomationUnit && setAutomationUnit(e.target.value)}>
